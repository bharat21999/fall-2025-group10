...
import shutil
from Component.utils import OUT_DIR as CMAB_OUT_DIR
# from utils import OUT_DIR as CMAB_OUT_DIR
...
def load_results(use_risk: bool):
    results = {}

    linucb_file = "linucb_history_risk.parquet" if use_risk else "linucb_history.parquet"
    linucb_label = "LinUCB (Risk)" if use_risk else "LinUCB"

    try:
        results[linucb_label] = pd.read_parquet(BASE_RESULTS_DIR / linucb_file)
        print(f"✓ Loaded {linucb_label}: {len(results[linucb_label]):,} rounds")
    except FileNotFoundError:
        print(f"✗ {linucb_label} results not found. Run main.py first.")
        results[linucb_label] = None

    # UCB1
    try:
        results["UCB1"] = pd.read_parquet(BASE_RESULTS_DIR / "ucb1_history.parquet")
    ...
    # Random
    try:
        results["Random"] = pd.read_parquet(BASE_RESULTS_DIR / "random_history.parquet")
    ...
    # CF
    try:
        results["CF"] = pd.read_parquet(BASE_RESULTS_DIR / "cf_history.parquet")
...
def get_assets_customers_from_cmab():
    """
    Read the CMAB events dataset and return (n_assets, n_customers).
    Uses the dataset built in utils.build_cmab_dataset_minimal().
    """
    cmab_path = CMAB_OUT_DIR / "cmab_events_min.parquet"

    if not cmab_path.exists():
        print(f"[get_assets_customers_from_cmab] CMAB dataset not found at: {cmab_path}")
        return None, None

    df = pd.read_parquet(cmab_path, columns=["ISIN", "customerID"])
    n_assets = int(df["ISIN"].nunique())
    n_custs = int(df["customerID"].nunique())

    print(f"[get_assets_customers_from_cmab] Assets: {n_assets}, Customers: {n_custs}")
    return n_assets, n_custs

def copy_summary_files(use_risk: bool):
    summary_files = [
        "ucb1_summary.csv",
        "cf_summary.csv",
        "random_summary.csv",
    ]
    linucb_summary = "linucb_summary_risk.csv" if use_risk else "linucb_summary.csv"
    summary_files.append(linucb_summary)

    for fname in summary_files:
        src = BASE_RESULTS_DIR / fname
        if src.exists():
            dst = OUT_DIR / fname
            shutil.copy2(src, dst)
            print(f"✓ Copied {fname} -> {dst}")
        else:
            print(f"✗ Summary file not found (skipped): {src}")
...
def main():
    ...
    # Ask only about risk-level LinUCB
    risk_input = input("\nUse LinUCB with risk level feature? (y/n): ").strip().lower()
    use_risk = risk_input in ["y", "yes"]

    # Load results from base results folder
    print("\nLoading results...")
    results = load_results(use_risk=use_risk)

    # Get assets/customers from CMAB dataset built via utils.py
    n_assets, n_custs = get_assets_customers_from_cmab()

    if n_assets is None or n_custs is None:
        scenario_name = "results_unknownassets_unknowncusts"
    else:
        scenario_name = f"results_{n_assets}assets_{n_custs}custs"

    if use_risk:
        scenario_name += "_withrisk"

    OUT_DIR = BASE_RESULTS_DIR / scenario_name
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"\nUsing scenario results directory: {OUT_DIR}")
    ...
    # Copy summaries into this scenario folder
    print("\nCopying summary CSV files into scenario directory...")
    copy_summary_files(use_risk=use_risk)

    print("\n" + "=" * 90)
    print("COMPARISON COMPLETE")
    print("=" * 90)
    print(f"\nAll plots and summaries saved under: {OUT_DIR}")
